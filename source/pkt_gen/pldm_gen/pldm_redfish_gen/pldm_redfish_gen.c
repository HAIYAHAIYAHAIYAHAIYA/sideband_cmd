...
static void pldm_redfish_gen_cmd_10(u8 *buf)
{
    ...
    if (g_cur_op_identify.op_flg & CBIT(1)) {
        ...
        root = pldm_cjson_create_networkadapter_v1_5_0_schema_update(dict, annc_dict);
...
static void pldm_redfish_gen_cmd_11(u8 *buf)
{
    ...
    req_dat->etag_cnt = 1;
    req_dat->collection_skip = 1;
    req_dat->collection_top = 1;
    req_dat->etag.format = UTF_8;
    req_dat->etag.len = 4;
    u8 *etag = "etag";
    req_dat->etag.len = cm_strlen(etag);
    cm_memcpy(req_dat->etag.val, etag, req_dat->etag.len);
}
