...
#define LINK_SPEED_10GB_FULL            PHY_SPEED_10GB
#define LINK_SPEED_1GB_FULL             PHY_SPEED_1GB
#define LINK_SPEED_100_FULL             PHY_SPEED_100MB
#define LINK_SPEED_10_FULL              PHY_SPEED_10MB
#define LINK_SPEED_UNKNOWN              PHY_SPEED_UNKNOWN
...
link_sts_t g_link_sts[MAX_LAN_NUM];    /* read from lan task */
 ...
 */
 u32 ncsi_calc_checksum(u8 *data, int len)
{
    u32 checksum = 0;
    int i;
...
void ncsi_tx2(protocol_msg_t *skb)
{
    ...
    LOG("pkt_len : %d", plen);
    for (u16 i = 0; i < (plen + 3) / 4; i++) {
        LOG("data : 0x%08x", ((u32 *)(skb->rsp_buf))[i]);
    }
}
