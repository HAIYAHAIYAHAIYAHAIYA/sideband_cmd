...
fw_pkt_data_fmt = {
    0x0000: ["Current_GFID", "Origin_GFID"],
    0x0001: ["VPD_key", "V0_Key_Data"]
}
...
def get_firmware_device_packagedata(firmware_device_package_data):
    pkt_data = bytes()
    for tlv in firmware_device_package_data:
        type = tlv["Type"]
        pkt_data += struct.pack("B", type)
        pkt_data += struct.pack("2H",
                                tlv[fw_pkt_data_fmt[type][0]],
                                tlv[fw_pkt_data_fmt[type][1]])
    # print(binascii.crc32(pkt_data))
    crc32 = binascii.crc32(pkt_data)
    pkt_data += crc32.to_bytes(4, "little")
    return pkt_data
...
def write_fw_device_identification_area(
    ...
    for device in devices:
        ...
        if 'FirmwareDevicePackageData' in device:
            firmware_device_package_data = device["FirmwareDevicePackageData"]
            package_data = get_firmware_device_packagedata(firmware_device_package_data)
            fw_device_pkg_data_length = len(package_data)
            record_length += len(package_data)
        ...
        if fw_device_pkg_data_length != 0:
            pldm_fw_up_pkg.write(package_data)
...
def write_component_image_info_area(pldm_fw_up_pkg, valid, metadata, image_files):
    """
    ...
    components = metadata["ComponentImageInformationArea"]
    component_cnt = 0
    if valid & macros.SLOT_IMG_VALID:
        component_cnt = 1
    if valid & macros.CHIP_IMG_VALID:
        component_cnt = 1
    if valid & macros.FACTORY_IMG_VALID:
        component_cnt = 1
    # ComponentImageCount
    pldm_fw_up_pkg.write(struct.pack("<H", component_cnt))
    ...
    for component in components:
        idx = component["ComponentIdentifier"]
        if image_files[idx][1] & valid:
    ...
    for offset in component_location_offsets:
        for idx in range(start_idx, len(image_files)):
            if image_files[idx][1] & valid:
                file_size = os.stat(image_files[idx][0]).st_size
...
def append_component_images(pldm_fw_up_pkg, valid, metadata, image_files):
    """
    ...
    for idx in range(len(image_files)):
        if image_files[idx][1] & valid:
            files = file_read_content(image_files[idx][0])
            pldm_fw_up_pkg.write(files)

def main(metadata, pldmfwuppkgname, valid):
    """Create PLDM FW update (DSP0267) package based on a JSON metadata file"""
    # Validate the number of component images
    if len(pldm_fwup_image_files) != len(metadata["ComponentImageInformationArea"]):
        sys.exit(
    ...
    try:
        with open(pldmfwuppkgname, "w+b") as pldm_fw_up_pkg:
            ...
            write_component_image_info_area(
                pldm_fw_up_pkg, valid, metadata, pldm_fwup_image_files
            )
            update_pkg_header_size(pldm_fw_up_pkg)
            write_pkg_header_checksum(pldm_fw_up_pkg)
            append_component_images(pldm_fw_up_pkg, valid, metadata, pldm_fwup_image_files)
            pldm_fw_up_pkg.close()
    except BaseException:
        pldm_fw_up_pkg.close()
        os.remove(pldmfwuppkgname)
        raise
...
if __name__ == "__main__":
    ...
    valid = get_config_val(metadata["valid"])
    for idx in range(len(pldm_fwup_output_files)):
        if pldm_fwup_output_files[idx][1] & valid:
            pldmfwuppkgname = pldm_fwup_output_files[idx][0]
            main(metadata, pldmfwuppkgname, pldm_fwup_output_files[idx][1])
