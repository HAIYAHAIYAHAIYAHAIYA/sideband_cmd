...
pldm_redfish_schema_info_t schema_info[ALL_SCHEMA_IDENTIFY] = {
    ...
    // [REGISTRY]                          = {SCHEMACLASS_REGISTRY,                            BIT(READ),                                          {"NetworkAdapter.v1_5_0.json"}},
...
static int schema_transfer_handle_get(u32 resource_id, u8 schemaclass)
{
    ...
    if ((resource_id == 0xFFFFFFFF) && (schemaclass == SCHEMACLASS_REGISTRY))
        ret = REGISTRY;
...
static void pldm_redfish_get_schema_dictionary(protocol_msg_t *pkt, int *pkt_len)
{
    ...
    gs_op_buf.multipartrecv_first_transfer_handle = resource_identify;
    rsp_dat->dictionary_fmt = 0x00;
    rsp_dat->transfer_handle = resource_identify;    // 后续当MC将此handle传输回来后，RDE设备通过将此handle作为schema/dictionary地址数组索引转换成地址后，将地址对应数据返回给MC
...
static void pldm_redfish_get_schema_uri(protocol_msg_t *pkt, int *pkt_len)
{
    ...
    if ((req_dat->oem_extension_num != 0) || (req_dat->requested_schemaclass == SCHEMACLASS_REGISTRY)) {
        rsp_hdr->cpl_code = PLDM_ERROR_INVALID_DATA;
        goto L_ERR;
    }
...
static void pldm_redfish_rde_operation_init(protocol_msg_t *pkt, int *pkt_len)
{
    ...
    if (!(req_dat->op_type & schema_info[resource_identify].allowed_op)) {      /* if operation not allowed for the requested resource (e.g. Update a RO resource). */
    ...
    if (req_dat->op_flg & CBIT(1)) {    // operation is Update or Replace, and have req_data
        if (req_dat->req_payload_len > 0) {
            cm_memcpy(gs_op_buf.op_data.data, (void *)(req_dat->op_locator + req_dat->op_locator_len), req_dat->req_payload_len);
            u32 crc = crc32_pldm(0xFFFFFFFFUL, gs_op_buf.op_data.data, gs_op_buf.op_data.len - sizeof(u32));
            if (crc != *(u32 *)&(req_dat->op_locator[req_dat->op_locator_len + req_dat->req_payload_len - sizeof(u32)])) {
...
static void pldm_redfish_supply_custom_request_parameters(protocol_msg_t *pkt, int *pkt_len)
{
    ...
    if (req_dat->etag_op == ETAG_IF_MATCH) {
        ...
        u8 *etag_string = NULL; // the calculated ETag
...
static void pldm_redfish_rde_multipart_send(protocol_msg_t *pkt, int *pkt_len)
{
    ...
    if ((req_dat->transfer_flg == PLDM_TRANSFER_FLG_START_AND_END) || (req_dat->transfer_flg == PLDM_TRANSFER_FLG_END)) {
        u32 crc = crc32_pldm(0xFFFFFFFFUL, gs_op_buf.op_data.data, (gs_op_buf.op_data.len - sizeof(u32)));
        if (crc != *(u32 *)(&gs_op_buf.op_data.data[gs_op_buf.op_data.len - sizeof(u32)])) {
            rsp_hdr->cpl_code = PLDM_ERROR_BAD_CHECKSUM;
...
static void pldm_redfish_rde_multipart_receive(protocol_msg_t *pkt, int *pkt_len)
{
    ...
    if (req_dat->transfer_op == XFER_FIRST_PART) {
        if (req_dat->op_id == 0) {                          // schema transfer
            // gs_op_buf.buf_ptr.data = (u8 *)dictionary;
    ...
    u16 max_pkt_len = g_pldm_redfish_base_info.mc_maximum_xfer_chunksize_bytes;
    u32 len = ((gs_op_buf.multipartrecv_next_transfer_handle + max_pkt_len - 4) >= gs_op_buf.buf_ptr.len) ? (gs_op_buf.buf_ptr.len - gs_op_buf.multipartrecv_next_transfer_handle) : (max_pkt_len - 4);
    rsp_dat->data_len_bytes = len;
    if ((gs_op_buf.multipartrecv_next_transfer_handle + max_pkt_len - 4) >= gs_op_buf.buf_ptr.len) {
        ...
        *(u32 *)&(rsp_dat->data[len]) = crc32_pldm(0xFFFFFFFFUL, gs_op_buf.buf_ptr.data, gs_op_buf.buf_ptr.len);
        *pkt_len += sizeof(u32);
...
u8 pldm_redfish_get_dict_data(u32 resource_id, u8 *dict, u16 len)
{
    ...
    //     if (PLDM_BASE_EVENT_DICT_RESOURCE_ID == dict_info->dict_info[i].resource_id) {
...
static u16 pldm_redfish_get_dict_len(u32 resource_id)
{
    u16 len = 0;
